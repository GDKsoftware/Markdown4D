<#
.SYNOPSIS
    Installs the MarkdownPad example and registers it as the handler for
...
.DESCRIPTION
    ...
    per-user file associations under HKCU:
      ...
      - an Applications entry so the pad shows up in "Open with"
...
.PARAMETER Flavour
    Which pad to register, Fmx (default) or Vcl.

.PARAMETER Extensions
    Extensions to associate. Defaults to .md only.

.PARAMETER InstallRoot
    Target directory. Defaults to %LOCALAPPDATA%\Programs\Markdown4D Pad.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File tools\Install-MarkdownPad.ps1

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File tools\Install-MarkdownPad.ps1 -Uninstall
#>
...
param(
    ...
    [string] $InstallRoot = (Join-Path $env:LOCALAPPDATA 'Programs\Markdown4D Pad'),
...
$AppName         = 'Markdown4D Pad'
$AppDescription  = 'Markdown editor with live preview, built on the Markdown4D component set.'
$BackupValueName = 'Markdown4DBackup'
$CapabilitiesKey = 'Software\Markdown4D Pad\Capabilities'

$Root = Split-Path -Parent $PSScriptRoot

if ($Flavour -eq 'Fmx') {
    $ProjectDir = Join-Path $Root 'Examples\MarkdownPadFMX'
    $ExeName    = 'MarkdownPadFMX.exe'
    $Payload    = @($ExeName, 'sk4d.dll')
}
else {
    $ProjectDir = Join-Path $Root 'Examples\MarkdownPadVCL'
    $ExeName    = 'MarkdownPadVCL.exe'
    $Payload    = @($ExeName)
}
...
function Register-Pad {
    if (-not $SkipCopy) {
        if (-not (Test-Path (Join-Path $BuildDir $ExeName))) {
            throw "Build not found: $(Join-Path $BuildDir $ExeName). Build the $Flavour pad in Release/Win64 first."
        }
        ...
        Write-Host "Installed $Flavour pad to $InstallRoot"
    }
...
function Unregister-Pad {
    ...
    Remove-Item -Path 'HKCU:\Software\Markdown4D Pad' -Recurse -Force -ErrorAction SilentlyContinue
    ...
    Update-ShellAssociations
    Write-Host 'Unregistered the pad.'
}

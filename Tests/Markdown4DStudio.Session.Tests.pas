unit Markdown4DStudio.Session.Tests;
...
begin
  FRootDir := TPath.Combine(TPath.GetTempPath, Format('Markdown4DStudio_%s', [TPath.GetGUIDFileName]));
...
begin
  const Resolved = TPadSession.ResolvePath('Markdown4DStudio.Vcl.json');

  Assert.IsTrue(Resolved.EndsWith(TPath.Combine('Markdown4D', 'Markdown4DStudio.Vcl.json')));
